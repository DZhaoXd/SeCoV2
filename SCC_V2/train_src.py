...
def train(cfg, local_rank, distributed):
    ...
    for i, (src_input, sam_masks, sam_psd_labels, sam_gt_labels, _, coord) in enumerate(train_loader):
        ...
        sam_logits, _ = classifier(features, sam_masks)
        
        # print('sam_psd_labels', sam_psd_labels.shape)
        # print('sam_logits', sam_logits.shape)
        
        sam_psd_labels = sam_psd_labels.view(-1)  # [B * num_regions]
        sam_gt_labels = sam_gt_labels.view(-1)  # [B * num_regions]
        #sam_loss = criterion(sam_logits, sam_gt_labels)
        sam_loss = criterion(sam_logits, sam_psd_labels)
        loss = sam_loss 
        # loss.backward()
        ...
        optimizer_fea.step()
        optimizer_cls.step()
        meters.update(sam_loss=sam_loss.item())
        iteration += 1
...
def run_test(cfg, model, local_rank, distributed):
    ...
    conn_acc_meter = AverageMeter()  # 用于跟踪连通域准确性
    class_acc_meter = {i: AverageMeter() for i in range(cfg.MODEL.NUM_CLASSES)}  # 每个类别的连通域准确性
    ...
    with torch.no_grad():
        for i, (x, sam_masks, sam_psd_labels, sam_gt_labels, _, coord) in enumerate(tqdm(test_loader, desc="Testing", ncols=100)):
            ...
            # 使用SAM分类器进行连通域分类
            # pred : [B * num_regions, num_class]
            fea = feature_extractor(x, sam_masks)
            pred, _ = classifier(fea, sam_masks)

            # 获取预测类别，通过 argmax 获取每个区域的预测类别
            num_classes = cfg.MODEL.NUM_CLASSES
            pred = pred.argmax(dim=1)  # [B * num_regions]
            
            # 将 sam_gt_labels 扁平化为 [B * num_regions]
            gt = sam_gt_labels.view(-1)                       # [N]
            valid = (gt != 255)
    
            # —— 总体 micro accuracy：按有效样本数加权更新 ——
            correct = (pred == gt) & valid
            num_correct = correct.sum().item()
            num_valid   = valid.sum().item()
            if num_valid > 0:
                conn_acc_meter.update(num_correct / num_valid, n=num_valid)  # 用 n 加权
    
            # —— 每类统计：同样用计数法并加权更新（避免批间权重不均）——
    ...
    # 汇总
    conn_acc = conn_acc_meter.avg  # 这是严格按样本数加权的 micro accuracy
    
    logger.info('Testing Results:')
    logger.info(f'Overall Accuracy (micro): {conn_acc:.4f}')
    
    # 另外给一个 macro（只在出现过的类别上求均值）
    class_accs, appeared = [], 0
    for i in range(cfg.MODEL.NUM_CLASSES):
        acc_i = class_acc_meter[i].avg
        if class_acc_meter[i].count > 0:   # 这个类在测试集中出现过
            appeared += 1
